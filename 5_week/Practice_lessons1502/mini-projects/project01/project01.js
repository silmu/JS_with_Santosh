...
const output = document.querySelector('#results-list');

//Can use 'change' listener instead too
search.addEventListener('keyup', (e) => {
  //console.log('Searching for ' + e.currentTarget.value);
  const found = data.filter((item) => {
    if (item.spacecraft.toLowerCase() === search.value.trim().toLowerCase()) {
      //console.log('Found:' + item.spacecraft);
      return item;
    }
  });
  found.length > 0
    ? renderResults(found[0].spacecraft)
    : console.log('No results found.');
});

const renderResults = (result = '') => {
  output.insertAdjacentHTML('beforeend', `<li>${result}</li>`);
};
...
//       if (item.spacecraft.toLowerCase() === search.value.trim().toLowerCase()) {
//         console.log('Found:' + item.spacecraft);
//         output.insertAdjacentHTML('beforeend', `<li>${item.spacecraft}</li>`);
//       }
//     });
//   });
